 ...
 */
    function pz_curl($url, $params, $post, $xyt=array(),$cookies=array(),$zdx=null,$fhxyt_COOKIES=null)
    {
        ...
        curl_setopt($ch, CURLOPT_TIMEOUT,10);//超时10秒设置
...
/**
 * @param $name string 文件路径 默认本文件目录
 * @param $data string 写入数据
 * @param $t int 1写入 2读取
 * @return int
 */
function pz_write($name, $data,$t)
{
    if ($t==1){
        $lj = dirname(__FILE__) . '/' . $name;
        $sz = file_put_contents($lj, $data);
        if ($sz == 0) {
            return -1;
        }
        return 1;
